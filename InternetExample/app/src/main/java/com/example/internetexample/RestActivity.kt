...
class MyViewModel : ViewModel() {
    ...
    val contributors= mutableMapOf<String,String>()
        ...
        viewModelScope.launch {
            try {
                ...
                response.value = StringBuilder().apply {
                    ...
                    repos.forEach {
                        contributors.put(it.name,it.owner.login)
                    }
            } catch (e: Exception) {
                if(contributors.containsValue(userName)){
                    response.value = StringBuilder().apply {
...
class RestActivity : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        myButton.setOnClickListener {buttonListner(userNameEdit.text.toString())}
        myViewModel.response.observe(this) {
            findViewById<TextView>(R.id.textResponse).text = it
        }
    }
    fun buttonListner(userName :String){
